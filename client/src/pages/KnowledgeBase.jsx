...
export default function KnowledgeBase() {
  ...
  function openEditTopic(t) {
    setForm({ key: t.key, title: t.title, content: t.content, law_refs: (t.law_refs || []).join(', '), keywords: (t.keywords || []).join(', ') })
    setEditing('topic')
  }
  ...
  async function saveTopic(e) {
    ...
    try {
      await api.put(`/dashboard/knowledge/topics/${form.key}`, {
        ...
        law_refs: form.law_refs.split(',').map(s => s.trim()).filter(Boolean),
  ...
  return (
    <div>
      <h2 className="mb-md">Base de Conocimientos Legales</h2>

      <div className="flex gap-sm mb-md">
        {[['topics', '📚 Temas Legales'], ['institutions', '🏛️ Instituciones'], ['services', '🏪 Servicios']].map(([key, label]) => (
              ...
              {topics.map(t => (
                <div key={t.key} className="card">
                  ...
                  <div className="flex gap-sm" style={{ flexWrap: 'wrap' }}>
                    {(t.law_refs || []).map((r, i) => <span key={i} className="badge badge-primary">{r}</span>)}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
            ...
            <form onSubmit={saveTopic}>
              ...
              <div className="mb-sm"><label className="label">Referencias legales (separadas por coma)</label><input className="input" value={form.law_refs || ''} onChange={e => setForm({ ...form, law_refs: e.target.value })} /></div>
