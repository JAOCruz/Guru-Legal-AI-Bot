...
const { detectIntent, isMenuChoice, detectDirectService } = require('./nlp');
...
async function routeMessage(phone, text, msg, savedMedia = null) {
  ...
  switch (session.flow) {
    ...
    case 'certificate':
      return await certificateFlow.handle(session, text);
...
async function handleMainMenu(session, text, msg, savedMedia = null) {
  ...
  if (step === 'show') {
    ...
    // Before intent switch, check for direct service mention
    const directService = detectDirectService(text);
    if (directService) {
      const { formatDirectServiceResponse, isNotarialService } = require('./flows/services');
      const response = formatDirectServiceResponse(directService.category, directService.itemName);
      if (response) {
        const notarial = isNotarialService(directService.category, directService.itemName);
        await transitionTo(session, 'services', 'direct_service', {
          directCategory: directService.category,
          directItem: directService.itemName,
          isNotarial: notarial,
        });
        return response;
      }
    ...
    switch (intent) {
      ...
      case 'certificate':
        ...
        return await certificateFlow.handle({ ...session, step: 'ask_purpose' }, text);
      case 'printing':
        await transitionTo(session, 'printing', 'ask_color');
        return await printingFlow.handle({ ...session, step: 'ask_color' }, text);
      case 'intake':
      case 'legal_info':
      case 'services':
      case 'appointment':
