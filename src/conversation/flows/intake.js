...
async function handle(session, text, msg) {
  ...
  switch (step) {
    ...
    case 'ask_name': {
      ...
      // Detect if user typed a question or non-name instead of their actual name
      const looksLikeQuestion = /[?¿]/.test(name) || /^(que|por ?que|para que|como|cuando|donde|cual|quien|necesito|quiero|ayuda)/i.test(normalize(name));
      if (looksLikeQuestion) {
        // They're confused or asking something — answer via LLM and offer to continue or go back
        if (config.gemini.enabled) {
          try {
            const { generateLegalResponse } = require('../../llm/generate');
            const llmResponse = await generateLegalResponse(text);
...
async function completeIntake(session) {
  ...
  try {
    ...
    await transitionTo(session, 'main_menu', 'show', {});
    return withList(MSG.INTAKE_SUCCESS(caseNumber) + '\n\n' + MSG.MAIN_MENU, LIST.MAIN_MENU);
  } catch (err) {
